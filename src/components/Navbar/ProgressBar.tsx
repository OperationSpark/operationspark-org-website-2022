...
const ProgressBar = ({ isTop, progressOnly = false }: ProgressBarProps) => {
  ...
  const handleClick = ({ id, top }: TNodeItem) => {
...
const getDomNodePos = (navHeight: number) => {
  ...
  let top = 0;
  const domNodePos = Array.from(document.querySelectorAll('._progress')).map(
    (node) => {
      const name = node.id
        .split('-')
      ...
      const { top: clientTop, height: clientHeight } =
        document.body.getBoundingClientRect();

      const fromTop = nodeTop - clientTop;
      const w =
        (nodeHeight / (clientHeight - window.innerHeight + clientTop)) * 100;
